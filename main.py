...
def setup_handlers(app_instance):
    ...
    try:
        # Імпортуємо всі необхідні обробники
        from handlers.profile import (
            start_profile_creation, show_my_profile, handle_main_photo, 
            handle_profile_message, handle_age_input, handle_gender_input,
            handle_seeking_gender_input, handle_city_input, handle_goal_input,
            handle_bio_input
        )
        from handlers.search import (
            search_profiles, search_by_city, show_next_profile, show_top_users,
            show_matches, show_likes, handle_top_selection, show_user_profile,
            handle_like_callback, handle_next_profile_callback, handle_like_back
        )
        from handlers.admin import (
            show_admin_panel, handle_admin_actions, show_users_list, show_banned_users,
            handle_broadcast_message, start_ban_user, start_unban_user, handle_ban_user,
            handle_unban_user, handle_user_search, show_bot_stats, update_database
        )
        ...
        # Обробник зв'язку з адміном
        app_instance.add_handler(MessageHandler(filters.Regex("^👨‍💼 Зв'язок з адміном$"), contact_admin))
        
        # Адмін обробники
        app_instance.add_handler(MessageHandler(filters.Regex('^👑 Адмін панель$'), show_admin_panel))
        app_instance.add_handler(MessageHandler(filters.Regex('^(📊 Статистика|👥 Користувачі|📢 Розсилка|🔄 Оновити базу|🚫 Блокування)$'), handle_admin_actions))
        app_instance.add_handler(MessageHandler(filters.Regex('^(📋 Список користувачів|🚫 Заблокувати користувача|✅ Розблокувати користувача|📋 Список заблокованих|🔙 Назад до адмін-панелі)$'), handle_admin_actions))
        ...
        app_instance.add_handler(CallbackQueryHandler(handle_admin_actions, pattern='^admin_'))
        
        # Фото обробник
        app_instance.add_handler(MessageHandler(filters.PHOTO, handle_main_photo))
        
        # Універсальний текстовий обробник (має бути останнім)
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("👋 Головне меню:", reply_markup=get_main_menu(user.id))
    except Exception as e:
...
async def universal_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Обробка скасування
        if text == "🔙 Скасувати" or text == "🔙 Меню":
            user_states[user.id] = States.START
        ...
        elif text == "👑 Адмін панель" and user.id == ADMIN_ID:
...
async def initialize_bot_async():
    ...
    try:
        ...
        # Ініціалізація інформації про бота
